...
import evs.comm.IAOR;
import evs.comm.InvocationStyle;
...
public class JavaCodeGenerator {
	...
	public void generateException(IIDLException exception) throws IOException {
		FileWriter writer = prepareWriter(exception.getPackage(), exception.getName());
		writer.write("public class " + exception.getName() + " extends evs.comm.RemotingException {\n");
	...
	private void writeProxyMethod(IIDLMethod method, FileWriter writer) throws IOException{
		...
		writer.write("\t\tList arguments = new ArrayList<Arguments>();\n");
		for(IIDLParameter param: method.getParameters()){
			writer.write("\t\targuments.add(" + param.getName() + ");\n");
		}
		writer.write("\t\ttry{\n");
		writer.write("\t\t\tevs.comm.InvocationObject object = new evs.comm.InvocationObject(\"TODO Reference\", \"");
	...
	private void createProxy(IIDLClass clazz) throws IOException{
		FileWriter writer = prepareWriter(clazz.getPackage(), (clazz.getName() + "Proxy"));
	...
	private void createRemoteObject(IIDLClass clazz) throws IOException{
		FileWriter writer = prepareWriter(clazz.getPackage(), (clazz.getName() + "Invoker"));
		...
		for(IIDLParameter parameter: clazz.getVariables()){
			writer.write("\tprivate static " + parameter.getType() + " " + parameter.getName() + ";");
		}
		writer.write("\n");
		for(IIDLMethod method: clazz.getMethods()){
			writeMethod(method, writer);
	...
	private void createInvoker(IIDLClass clazz) throws IOException{
		...
		writer.write("\t\tevs.comm.InvocationObject object = marshaller.unmarshall(message);\n");
