...
public class LifecycleManager implements ILifecycleManager {
	 ...
	 */
	public synchronized IRemoteObject invocationArrived(IObjectReference ref) {
		...
		switch(strategy){
			...
			case CLIENT_PASSIVATION:
				if(ref.getInstanceId().length() == 0) return null;
			...
			case CLIENT:
				if(ref.getInstanceId().length() == 0) return null;
				if(this.clientInstances.get(ref.getClientDependent()) == null) return null;
				this.clientInstances.get(ref.getClientDependent()).setLocked(true);
				return this.clientInstances.get(ref.getClientDependent()).getObject();	
			case LAZY:
				//instantiate object if not already available
				IObjectState state = this.staticInstances.get(key);
				if(state == null){
					try {
						instance = (IRemoteObject) (Class.forName(ref.getObjectId())).newInstance();
	 ...
	 */
	public synchronized void invocationDone(IObjectReference ref, IRemoteObject object){
		String key = ref.getClientIndependent();
		LifecycleStrategy strategy = this.registeredObjects.get(key);
	...
	//--------------------------------------------------------------------- REGISTER OBJECTS AND STRATEGIES
	 ...
	 * @throws IllegalObjectException
	 */
	public synchronized void register(IObjectReference ref, LifecycleStrategy strategy) throws IllegalObjectException {		
		if((strategy == LifecycleStrategy.STATIC) ||
		   (strategy == LifecycleStrategy.STATIC_PASSIVATION)){
			try {
				...
				this.staticInstances.put(ref.getClientIndependent(), new ObjectState(instance));
			} catch (Exception exception){
	 ...
	 */
	public synchronized void unregister(IObjectReference ref){
	...
	public synchronized Integer newInstance(IObjectReference ref) throws NotSupportedException, IllegalObjectException{
	...
	public synchronized void keepAlive(IObjectReference ref) throws NotSupportedException, IllegalObjectException{
		LifecycleStrategy strategy = this.registeredObjects.get(ref.getClientIndependent());
	...
	public synchronized void destroy(IObjectReference ref) throws NotSupportedException, IllegalObjectException{
		LifecycleStrategy strategy = this.registeredObjects.get(ref.getClientIndependent());
	...
	private boolean passivateObject(String name, IRemoteObject object){
		try {
			System.out.println("[*] passivating " + name);
